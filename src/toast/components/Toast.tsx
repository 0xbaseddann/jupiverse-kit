...
>(({ className, variant = "default", ...props }, ref) => {
  return (
    <Toast.Root
      ref={ref}
      className={`
        jupiverse-toast group pointer-events-auto relative flex w-full items-center justify-between space-x-2 overflow-hidden rounded-md border p-4 pr-6 shadow-lg transition-all
        ...
        ${
          variant === "default"
            ? "border bg-white dark:bg-zinc-950 text-black dark:text-white"
            : ""
        }
        ${
          variant === "destructive"
...
>(({ className, ...props }, ref) => (
  <Toast.Description ref={ref} className="text-sm opacity-90" {...props} />
));

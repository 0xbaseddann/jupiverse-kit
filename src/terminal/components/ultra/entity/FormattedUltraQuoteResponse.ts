...
const AmountFromString = coerce<JSBI, null, string>(
  instance(
    JSBI.BigInt(0).constructor as unknown as { new (...args: any[]): any }
  ),
  string(),
  (value) => JSBI.BigInt(value)
);

const NumberFromString = coerce<string, null, number>(
  string(),
  number(),
  (value) => Number(value)
);
...
export type FormattedUltraQuoteResponse = Infer<
  typeof FormattedUltraQuoteResponse
>;
